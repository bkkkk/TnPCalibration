...
IFitter::IFitter(std::string fitterName,
                 std::string fittingFunctionName,
                 TH1F* histogramToFit,
                 const FitConfig& configuration)
    : name(std::move(fitterName)),
      fitConfig(configuration),
      functionName(std::move(fittingFunctionName)),
      histogram(histogramToFit),
      histogramName{},
      ...
      background{functionName + "_Bkg_" + histogramName,
                 ...
                 bottomFitLimit,
                 topFitLimit},
      composite{functionName, fitConfig.GetCompositeFunction() ,signal, background},
      compositeUp{functionName + "_Composite_Up_" + histogramName,
                  fitConfig.GetCompositeFunction(),
                  signal,
                  background},
