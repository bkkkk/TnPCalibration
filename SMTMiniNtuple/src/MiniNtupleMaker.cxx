...
namespace MIN {
  ...
  initialize ()
  {
    ...
    out_tracks.SetActive(kTRUE, "^trk_n$|^trk_pt$|^trk_eta$|^trk_phi_wrtPV$|^trk_chi2$|^trk_d0_wrtPV$|^trk_z0_wrtPV$|^trk_expectBLayerHit$|^trk_nBLHits$|^trk_nPixHits$|^trk_nPixHoles$|^trk_nPixelDeadSensors$|^trk_nSCTHits$|^trk_nSCTDeadSensors$|^trk_nSCTHoles$|^trk_nTRTHits$|^trk_nTRTOutliers$|^trk_ndof$|^trk_qoverp_wrtPV$|^trk_theta_wrtPV$");
    if (output)
    {
  ...
  execute ()
  {
    ...
    // Turn on debug flag
    if(eventCounter == eventDebug) SET_LOG_LEVEL(kWARNING);
    ...
    for(size_t trigIdx = 0; trigIdx != triggerList.size(); trigIdx++)
    {
      if(tdt->GetConfigSvc().IsConfigured(triggerList[trigIdx]) == 1)
      {
        if(tdt->IsPassed( triggerList[trigIdx] ) != 0)
        {
          ...
          passedTrigger = true;
        }; // End if trigger passed
      }; // end if trigger configured
    }; // end for trigger list
